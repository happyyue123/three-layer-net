...
class ThreeLayerNet(object):
    def __init__(
        ...
        self.params["W1"] = std * torch.randn(
            input_size, hidden1_size, dtype=dtype, device=device
        )
        self.params["b1"] = torch.zeros(hidden1_size, dtype=dtype, device=device)
        self.params["W2"] = std * torch.randn(
            hidden1_size, hidden2_size, dtype=dtype, device=device
        )
        self.params["b2"] = torch.zeros(hidden2_size, dtype=dtype, device=device)
        self.params["W3"] = std * torch.randn(
            hidden2_size, output_size, dtype=dtype, device=device
        )
        self.params["b3"] = torch.zeros(output_size, dtype=dtype, device=device)
...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    #ReLu 
    hidden1 = torch.maximum(s1, torch.zeros_like(s1)) # 计算第一个隐藏层 
    #
    s2 = torch.matmul(hidden1, W2) + b2
    #ReLU
    hidden2 = torch.maximum(s2, torch.zeros_like(s2)) # 计算第二个隐藏层
    #
    scores = torch.matmul(hidden2, W3) + b3
